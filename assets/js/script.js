      ...
      response.json().then(function (data) {
        ...
        for (var i = 0; i < data.list.length; i++) {
          ...
          if (data.list[i].dt_txt.indexOf("15:00:00") !== -1) {
            ...
            let windSpeedEl = document.createElement("p");
            let icon = document.createElement("img");
            dateEl.innerText = new Date(data.list[i].dt_txt).toLocaleDateString();
            cityEl.innerText = data.city.name;
            tempEl.innerText = "The Temperature: " + data.list[i].main.temp +" °C";
            humidityEl.innerText = "Humidity: " + data.list[i].main.humidity + "%";
